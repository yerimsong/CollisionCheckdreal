"""
Collision Checking with Multiple Robots Using dreal and Dynamic Window Approach
"""
...
def find_traj_win(x, config, predict_time):

    dw = calc_dynamic_window(x, config)

    return calc_traj_win(x, dw, config, predict_time)

def calc_traj_win(x, dw, config, predict_time):
    
    # in order to check for collision among robots using dreal, create a trajectory window for each robot

    x_init = x[:]
    xmin, xmax, ymin, ymax = x_init[0], x_init[0], x_init[1], x_init[1]

    for v in np.arange(dw[0], dw[1], config.v_reso):
        for y in np.arange(dw[2], dw[3], config.yawrate_reso):

            traj = predict_trajectory(x_init, v, y, config, predict_time)
            temp_xmin = np.min(traj[:, 0])
            temp_xmax = np.max(traj[:, 0])
            temp_ymin = np.min(traj[:, 1])
            temp_ymax = np.max(traj[:, 1])

            if temp_xmin < xmin: xmin = temp_xmin
            if temp_xmax > xmax: xmax = temp_xmax
            if temp_ymin < ymin: ymin = temp_ymin
            if temp_ymax > ymax: ymax = temp_ymax

    return [xmin, xmax, ymin, ymax]

def dwa_control(x, config, goal, ob, xs, x_i, reach_goal, traj_win):
    """
...
def calc_control_and_trajectory(x, dw, config, goal, ob, xs, x_i, reach_goal, traj_win):
    """
    ...
    while predict_time >= 0:
        ...
        for v in np.arange(dw[0], dw[1], config.v_reso):
            for y in np.arange(dw[2], dw[3], config.yawrate_reso):
                ...
                ob_cost = config.obstacle_cost_gain * calc_obstacle_cost(trajectory, ob, config, xs, x_i, reach_goal, 
                                                                         traj_win[predict_time])
        ...
        for i in range(len(xs)):
            if i != x_i and reach_goal[i] != 1:
                ...
                f_sat = dreal.And(min_dx <= my_x, max_dx >= my_x,
                                  ...
                                  traj_win[i][0] <= oth_x, traj_win[i][1] >= oth_x,
                                  traj_win[i][2] <= oth_y, traj_win[i][3] >= oth_y,
...
def main(gx=10.0, gy=10.0, robot_type=RobotType.circle):
    ...
    traj_win = {}
    while True:
        # create window for trajectories, which will be used for dreal collision checking
        predict_time = config.predict_time
        while predict_time >= 0:
            traj_win[predict_time] = [[0, 0, 0, 0]] * len(x)
            for i in range(len(x)):
                traj_win[predict_time][i] = [float(j) for j in find_traj_win(x[i], config, predict_time)]
            predict_time = predict_time - 1
        ...
        for i in range(len(x)):
            if reach_goal[i] == 1:
                continue
            u[i], predicted_trajectory[i] = dwa_control(x[i], config, goal, ob, x, i, reach_goal, traj_win)
